...
public class VaultManager {
    ...
    public VaultManager() {
        if (!WITSNMain.getInstance().getConfigManager().isRewardVault()) {
            economyEnabled = false;
            return;
        }

        if (Bukkit.getPluginManager().isPluginEnabled("Vault")) {
            WITSNMain.getInstance().getLogger().log(Level.WARNING, "Vault is not installed, so rewards are disabled!");
            economyEnabled = false;
            return;
        }
        ...
        if (!economyEnabled) {
            WITSNMain.getInstance().getLogger().log(Level.INFO, "No economy system was found connected to Vault, so rewards are disabled!");
        }

        WITSNMain.getInstance().getLogger().log(Level.INFO, "Vault has been linked to " + economy.getName() + "!");
    }
